...
module.exports = (() => {
  ...
  return {
    ...
    ReportanlaysisDataTable: async (body, res) => {
      try {
        console.log(
          "GE F",
          JSON.stringify(body, null, 2)
        ...
        const response = await sapAxios.post(
          config.THIRD_PARTY_API_URL_POST_Report,
          body,
          {
            headers: {
              Authorization: getAuthHeader()
    ...
    AddUser: async (body, res) => {
      try {
        ...
        const response = await sapAxios.post(
          config.THIRD_PARTY_API_URL_POST_Adduser,
          body,
          {
            headers: {
              Authorization: getAuthHeader()
    ...
    DisplayTable: async (body, res) => {
      try {
        // console.log(
        //   "Sending GET payload to plant API:",
        //   JSON.stringify(body, null, 2)
        // );
        const response = await axios.get(config.THIRD_PARTY_API_URL_GET_DisplayTable, {
          headers: {
            Authorization: getAuthHeader(),
          },
        });
    ...
    fetch_Exit_Cancel: async (body, res) => {
      try {
        console.log(
          "GE F",
          JSON.stringify(body, null, 2)
        ...
        const response = await sapAxios.post(
          config.THIRD_PARTY_API_URL_POST_fetch_Exit_Cancel,
          body,
          {
            headers: {
              Authorization: getAuthHeader()
   ...
   save_Exit_Cancel: async (body, res) => {
      try {
        ...
        const response = await sapAxios.post(
          config.THIRD_PARTY_API_URL_POST_save_Exit_Cancel,
          body,
          {
            headers: {
              Authorization: getAuthHeader()
        ...
        handleAxiosError(error, "save exit and cancel change Gate Entry");
        res.status(500).json({ error: "fetch change Gate Entry" });
      }
    },
 ...
 UserEdit: async (body, res) => {
      try {
        ...
        const response = await sapAxios.post(
          config.THIRD_PARTY_API_URL_POST_Edit,
          body,
          {
            headers: {
              Authorization: getAuthHeader()
